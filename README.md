...
which is in the securesystemslib json format:
```console
...
-----END EC PRIVATE KEY-----
```
So we need to take the contents and base64 decode it:
```console
$ cat input
MHcCAQEEIFjdJCw6Lkx1VPYtdnbihRKoLFb36zzAc0XgCJ1B5/oPoAoGCCqGSM49
AwEHoUQDQgAEY62fGO3T7D69Hmu58+/QcDAXB30Wzh84kXRBNviAkNyUf5hVVXcH
...
$ base64 -d input > input.der

$ openssl ec -in input.der -inform=der -noout -text
read EC key
Private-Key: (256 bit)
priv:
    58:dd:24:2c:3a:2e:4c:75:54:f6:2d:76:76:e2:85:
...
NIST CURVE: P-256
```
Now, notice that there are newlines in the input file. If we use `\n` is
the file then we will get an error `base64: invalid input`.

The key that is generate by cosign is in the following format:
```console
$ cat ../cosign.key
-----BEGIN PRIVATE KEY-----
MIGHAgEAMBMGByqGSM49AgEGCCqGSM49AwEHBG0wawIBAQQgWN0kLDouTHVU9i12
...
-----END PRIVATE KEY-----
```
And we can convert this key from pem to der using the following command:
```console
$ openssl ec -inform=pem -in ../cosign.key --outform=der --out cosign.der
$ ls -l cosign.der 
-rw-------. 1 danielbevenius danielbevenius 121 Dec  7 08:00 cosign.der
```

I was expecting that if we took the "raw" contents of the pem file, base64
decode it into binary, this binary should be identical to what we did above on
the command line.
But looking at the produces binary this is not the case:
...
```
$ openssl pkcs8 -in priv.pem -out priv.pkcs8 -topk8 -nocrypt
$ cat priv.pkcs8 
-----BEGIN PRIVATE KEY-----
MIGHAgEAMBMGByqGSM49AgEGCCqGSM49AwEHBG0wawIBAQQgWN0kLDouTHVU9i12
duKFEqgsVvfrPMBzReAInUHn+g+hRANCAARjrZ8Y7dPsPr0ea7nz79BwMBcHfRbO
...
openssl-rs for this which is what I've opted for. 

So that will allow the create-layout.rs to pass and sign the layout, but we
still have an issue with the generated keyid's which is a reoccuring theme.
